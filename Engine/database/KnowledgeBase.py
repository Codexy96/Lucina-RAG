...
class KnowledgeBase:
    ...
    async def initialize(self):
        ...
        print("知识库初始化成功，使用{}向量tensorrt引擎。".format(self.embedding.name))
    ...
    def insert_data(self, name, data):
        if name not in self.kb_dict:
            ...
            if name in self.milvus.show():
                ...
                self.milvus.create(name=name, fields=milvus_fields,core_dict={'method':'IVF_SQ8','dim':self.embedding.dim})
    ...
    async def search(self, name, query, threshold=3, top_k=10):
        embedding_list = self.embedding(query)
        if name not in self.kb_dict:
            print("该知识库不存在！")
            return None

        milvus_result = self.milvus.search(name, embedding_list, top_k=top_k)
        es_result = await self.es.search(name, query, threshold=threshold, top_k=top_k)
