...
async def check_url(url, timeout=1):
    async with aiohttp.ClientSession() as session:
        try:
...
async def process_results(results):
    ...
    # 检查每个 URL 的有效性
    url_check_tasks = [check_url(url) for url in urls]
...
def  doc_process(docs):        
        ...
        for i, doc in enumerate(docs_transformed):
            ...
            for index in range(0, len(search_content), 512):
                result.append(
                    {
                        'content': search_content[index:index+512] + f'\n<llmlingua, compress=False>标题：{title_content}\t链接：{docs[i].metadata["source"]}\t简介：{snippet}\t来源：{source}</llmlingua>',
                        'metadata': metadata[i]
                    }
                )
        return result
