...
from pyecharts.charts import Bar, Line, Pie, Liquid, Scatter, WordCloud,Graph,Grid
flie_path=os.path.dirname(__file__)
config_path=os.path.join(flie_path,'../../Setup/config.yaml')
import yaml 
import json
...
class doubaoAI:
    ...
    def yield_output(self,response):
        for chunk in response:
            yield chunk.choices[0].delta.content
    def chat(self,text,model=config['doubaoAI']['ep_point'],stream=False,**kwargs):
        """
        参数：
        text:输入文本
        ...
        if not stream:
            ...
            return self.yield_output(completion)
    def apply(self,messages,model=config['doubaoAI']['ep_point'],**kwargs):
        completion=self.client.chat.completions.create(
            model=model,
            messages=messages,
            stream=False,
        ...
        messages.append({'role':role,'content':content})
        return messages
    def response(self,messages,model=config['doubaoAI']['ep_point'],stream=True,**kwargs):
        completion=self.client.chat.completions.create(
            model=model,
            messages=messages,
            stream=stream,
        ...
        for chunk in response:
            ...
            if code_start_pattern.match(content):
                ...
                if in_code_block:
                    ...
                    yield content
    def response_code(self,messages,model=config['doubaoAI']['ep_point'],stream=True,**kwargs):
        completion=self.client.chat.completions.create(
            model=model,
            messages=messages,
            stream=stream,
