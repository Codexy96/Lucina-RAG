...
class zhipuAI:
    ...
    def apply(self,messages:list[dict],model:str='glm-4-air',**kwargs):
        """
        参数：
        ...
        return messages
    def response(self,messages:list[dict],model:str='glm-4-air',stream=False,**kwargs):
        """
        参数：
        messages:输入的message列表
        model：模型型号，默认为glm-4-air
...
class aliAI:
    ...
    def apply(self,messages:list[dict],model:str='qwen-plus',**kwargs):
        completion=self.client.chat.completions.create(
            model=model,
            messages=messages,
            stream=False,
        ...
        messages.append({'role':role,'content':content})
        return messages
    def response(self,messages:list[dict],model:str='qwen-plus',stream=True,**kwargs):
        completion=self.client.chat.completions.create(
            model=model,
            messages=messages,
            stream=stream,
...
class deepseekAI:
    ...
    def chat(self,text,model="deepseek-chat",stream=False,**kwargs):
        ...
        if not stream:
            ...
            return self.yield_output(completion)
    def apply(self,messages:list[dict],model:str='deepseek-chat',**kwargs):
        completion=self.client.chat.completions.create(
            model=model,
            messages=messages,
            stream=False,
        ...
        messages.append({'role':role,'content':content})
        return messages
    def response(self,messages:list[dict],model:str='deepseek-chat',stream=True,**kwargs):
        completion=self.client.chat.completions.create(
            model=model,
            messages=messages,
            stream=stream,
