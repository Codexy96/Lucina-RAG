"""
该脚本专门适用于对具有标题层级结构的文本进行拆分。 

具体步骤如下：

1、对文本进行切分，生成多个小段落

2、检测每个切分后的段落是否为标题，并为其添加相应的元数据

3、位于一个标题下的段落尽量合并成一个大段落，保持段落的完整性，如果超过字数限制，则拆分，为非标题文本添加与标题相关的上下文

4、每个文本块进行上下文重叠，保留一定上下文语义

"""

""" 
使用方法：
from splite4title import TitleStructuredTextProcessor
sample_texts = [
       ""反分裂国家法
...
processor = TitleStructuredTextProcessor(block_size=100, context_overlap=1)
documents = processor.create_documents(sample_texts)
...
class TitleStructuredTextProcessor(CharacterTextSplitter):
    ...
    def __init__(self, block_size: int = 100, context_overlap: int = 2, **kwargs):
        super().__init__(**kwargs)
        self.block_size = block_size  # 块的宽松限制长度
        self.context_overlap = context_overlap  # 重叠的句子个数
            ...
            for index,chunk in enumerate(chunks):
                ...
                if is_possible_title(chunk):
                    if title and temp_content:
                        ...
                        self._add_document(documents, temp_content, _metadatas[i], title,next_sentence=chunks[index+1:index+self.context_overlap+1] if index+self.context_overlap+1<len(chunks) else chunks[index+1:])
                        temp_content =""  
                        title=None  # 重置标题
                    # 更新当前标题和内容
                    title = chunk
    ...
    def _add_document(self, documents: List[Document], combined_content: str, metadata: dict, title: str,next_sentence:str=None):
        """添加文档,并处理后缀重叠逻辑"""
...
""" 
返回结果：
langchain document类的列表，document.page_content为文本内容，document.metadata为元数据

document.page_content示例: 第一条 为了发展慈善事业，弘扬慈善文化，规范慈善活动，保护慈善组织、捐赠人、志愿者、受益人等慈善活动参与者的合法权益，促进社会进步，共享发展成果，制定本法。

document.metadata示例: {'category': 'cn_Title', 'title': '慈善法第一章总则'}
